...
type Agente struct {
	...
	/**
	 * Trabajos que ya se imprimieron, aunque no se lograra confirmar.
	 *
	 * El servidor reencola a los dos minutos lo que nadie confirma, que
	 * está bien para cuando el PC se apaga a mitad. Pero si la
	 * confirmación falla por otro motivo, el mismo ticket sale una y
	 * otra vez hasta agotar los reintentos.
	 *
	 * Con un rollo de papel de por medio, eso no es un detalle.
	 */
	impresos map[int]time.Time
}
...
func Nuevo(cfg *config.Config, registro *log.Logger) *Agente {
	return &Agente{
		...
		registro: registro,
		impresos: make(map[int]time.Time),
	}
}
...
func (a *Agente) procesar(trabajo api.Trabajo, cfg api.Config) {
	/**
	 * Si ya se imprimió, no se repite: solo se reintenta confirmar.
	 *
	 * Es la diferencia entre gastar un papel y gastar el rollo entero.
	 */
	if cuando, ya := a.impresos[trabajo.ID]; ya {
		a.registro.Printf(
			"Trabajo %d ya se imprimió a las %s; solo se reintenta confirmar",
			trabajo.ID, cuando.Format("15:04:05"))

		if err := a.cliente.Confirmar(trabajo.ID, true, ""); err == nil {
			delete(a.impresos, trabajo.ID)
		}
	...
	if err := a.cliente.Confirmar(trabajo.ID, true, ""); err != nil {
		...
		// Queda anotado para no repetirlo si el servidor lo reencola
		a.impresos[trabajo.ID] = time.Now()
...
/**
 * Se olvida lo impreso hace más de una hora.
 *
 * Sin esto la lista crecería sin fin en un equipo que lleve meses
 * encendido. Y una hora es de sobra: si un trabajo sigue sin confirmarse
 * pasado ese tiempo, el servidor ya lo habrá dado por perdido.
 */
func (a *Agente) limpiarImpresos() {
	limite := time.Now().Add(-time.Hour)

	for id, cuando := range a.impresos {
		if cuando.Before(limite) {
			delete(a.impresos, id)
		}
	}
}
