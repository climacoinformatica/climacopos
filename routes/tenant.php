...
use App\Http\Controllers\Panel\DocumentosController;
	...
	Route::get('produccion/exportar', [ProduccionController::class, 'exportar'])->name('produccion.exportar');
        ...
        Route::middleware('terminal')->group(function () {
		    ...
		    Route::post('tpv/{ticket}/imprimir', [TpvController::class, 'imprimir'])->name('tpv.imprimir');
                ...
                /**
                 * Reimprimir un cierre.
                 *
                 * Hace falta mas de lo que parece: la impresora se queda
                 * sin papel a mitad, o el conector estaba parado, y el
                 * cierre ya no se puede repetir porque los tickets estan
                 * marcados.
                 */
                Route::post('caja/cierre/{cierre}/imprimir', [CajaController::class, 'reimprimir'])
                    ->middleware('permiso:caja.cierre')->name('caja.reimprimir');

                // ---- Documentos en PDF
                Route::get('documentos/cierre/{cierre}', [DocumentosController::class, 'cierrePdf'])
                    ->middleware('permiso:caja.cierre')->name('documentos.cierre.pdf');
                Route::post('documentos/cierre/{cierre}/enviar', [DocumentosController::class, 'cierreEnviar'])
                    ->middleware('permiso:caja.cierre')->name('documentos.cierre.enviar');

                Route::get('documentos/facturas', [DocumentosController::class, 'facturas'])
                    ->middleware('permiso:informes.ver')->name('documentos.facturas');
                Route::get('documentos/facturas/pdf', [DocumentosController::class, 'facturasPdf'])
                    ->middleware('permiso:informes.ver')->name('documentos.facturas.pdf');
                Route::post('documentos/facturas/enviar', [DocumentosController::class, 'facturasEnviar'])
                    ->middleware('permiso:informes.ver')->name('documentos.facturas.enviar');
