...
class ConstructorTicket
{
     ...
     *
     * Es el mismo arqueo que el cierre, pero SIN cerrar nada: no crea
     * cierre, no marca tickets y se puede sacar las veces que haga
     * falta. Sirve para cuadrar la caja a media manana o cuando cambia
     * el turno, sin tocar la contabilidad del dia.
     *
     * Recibe el array que devuelve GestorCierre::resumen().
     */
    public function informeX(array $resumen, ?string $usuario = null): string
    {
        $d = $this->diseno;
        $esc = new EscPos($d->columnas);
        ...
        $esc->fila('Desde', $resumen['desde']->format('d/m/Y H:i'));
        $esc->fila('Emitido', now()->format('d/m/Y H:i'));
        ...
        $esc->fila('Tickets', (string) $resumen['num_tickets']);
        $esc->fila('Base', $this->euros((float) $resumen['total_base']));
        $esc->fila('Impuesto', $this->euros((float) $resumen['total_impuesto']));
        $esc->negrita(true)
            ->fila('VENTAS', $this->euros((float) $resumen['total_ventas']))
            ->negrita(false);
        $esc->fila('Ticket medio', $this->euros((float) $resumen['ticket_medio']));
        ...
        if ($resumen['por_medio'] !== []) {
            ...
            foreach ($resumen['por_medio'] as $medio => $importe) {
                $esc->fila(
                    ...
                    $this->euros((float) $importe),
                );
            }
        }

        $esc->separador()->negrita(true)->linea('EFECTIVO EN CAJA')->negrita(false);
        $esc->fila('Fondo inicial', $this->euros((float) $resumen['efectivo_inicial']));
        $esc->fila('Ventas efectivo', $this->euros((float) ($resumen['por_medio']['EFECTIVO'] ?? 0)));

        if ((float) $resumen['entradas'] > 0) {
            $esc->fila('Entradas', $this->euros((float) $resumen['entradas']));
        }

        if ((float) $resumen['salidas'] > 0) {
            $esc->fila('Salidas', '-' . $this->euros((float) $resumen['salidas']));
        }

        $esc->negrita(true)
            ->fila('DEBE HABER', $this->euros((float) $resumen['efectivo_teorico']))
            ->negrita(false);

        if (! empty($resumen['por_familia'])) {
            $esc->separador()->negrita(true)->linea('POR FAMILIA')->negrita(false);

            foreach ($resumen['por_familia'] as $nombre => $importe) {
                $esc->fila($nombre, $this->euros((float) $importe));
            }
        }

        if (! empty($resumen['por_profesional'])) {
            $esc->separador()->negrita(true)->linea('POR PROFESIONAL')->negrita(false);

            foreach ($resumen['por_profesional'] as $nombre => $importe) {
                $esc->fila($nombre, $this->euros((float) $importe));
            }
        }
         ...
         *
         * Un X y un Z se parecen demasiado en papel. Si alguien archiva
         * un X creyendo que cerro la jornada, al dia siguiente el cierre
         * real sale con el doble de ventas y nadie entiende por que.
         */
        ...
        if ($resumen['formacion'] > 0) {
            $esc->saltos(1)->linea($resumen['formacion'] . ' doc. de formacion aparte');
        }
